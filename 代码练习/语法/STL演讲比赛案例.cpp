...
//产生选手，map是一个编号对应一个选手 
int GenSpeaker(map<int, Speaker> &mapSpeaker, vector<int> &v)//v对应人员编号 
{
	string str = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
	random_shuffle(str.begin(), str.end());//随机打乱str 

	for (int i=0; i<24; i++)
	{
		Speaker tmp;
		tmp.m_name = "选手";
		tmp.m_name = tmp.m_name + str[i];//选手名字 
		mapSpeaker.insert(pair<int, Speaker>(100+i, tmp) );//插入键值对 
	}

	for (int i=0; i<24; i++)
	{
		v.push_back(100+i); //参加比赛的人员顺序，初始按照编号从小到大抽签 
	}
	return 0;
}
...
{
	random_shuffle(v.begin(), v.end());//索引小的先被抽到 
	return 0;
}


//选手比赛,index表示比赛的场次，v1表示参赛的人员，v2表示晋级的人员 ，mapSpeaker表示参赛的选手名单 
int speech_contest(int index, vector<int> &v1, map<int, Speaker> &mapSpeaker, vector<int> &v2 )
{
	//小组的比赛得分 记录下来;求出前三名 后3名
	multimap<int, int, greater<int> > multmapGroup; //小组成绩,允许键值对重复 ，greater升序排序 
	int tmpCount = 0;
	...
	for (vector<int>::iterator it=v1.begin(); it!=v1.end(); it++ )
	{
		...
		//打分
		{
			...
			//求平均分
			int scoresum = accumulate(dscore.begin(), dscore.end(), 0);//总分累积 
			int scoreavg = scoresum/dscore.size();
			mapSpeaker[*it].m_score[index] = scoreavg; //选手得分 存入容器中,索引取值 
			multmapGroup.insert(pair<int ,int>(scoreavg, *it ) );//存入人员编号对应的成绩 
		}
