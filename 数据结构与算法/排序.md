...
{
    for(int i = 1;i < n;i++)
        for(int j = i;j >= 1 && A[j] < A[j-1];j--)		//每次在子序列中找插入的位置
            swap(A,j,j-1);
}
```
...
{
    for(int i = 0;i < n - 1;i++)				//冒泡多少次
        for(int j = n - 1;j > i;j--)			//比较的次数
            if(A[j] < A[j-1])					//每次找到最小的给冒出来，走的比较慢，因为得俩俩交换，谁小冒谁，类似选择排序，但是冒泡是稳定的
                swap(A,j,j-1);					//前面的已经放好，只需要管后面的
}
```

### 性能

冒泡排序是一种相对较慢的排序，没有较好的最佳情况执行时间。通常情况下**时间复杂度都是O(n^2)**
...
{
    bool flag=false;
    for(int i = 0;i < n - 1;i++)				//冒泡多少次，flag在此处添加
        flag=false;
        for(int j = n - 1;j > i;j--)			//比较的次数
            if(A[j] < A[j-1]){
                swap(A,j,j-1);
...
第i次“选择”数组中第i小的记录，并将该记录放到数组的第i个位置。换句话说，每次从未排序的序列中找到最小元素，放到未排序数组的最前面，每次都是找最小的直接拿出来放前面。
...
不管数组是否有序，在从未排序的序列中查找最小元素时，都需要遍历完最小序列，所以**时间复杂度为O(n^2)**
...
shell排序在**不相邻**的元素之间比较和交换。利用了插入排序的最佳时间代价特性，**它试图将待排序序列变成基本有序的，然后再用插入排序来完成排序工作**
...
![1565611612482](pic/1565611612482.png)

### 代码

```c++
const int INCRGAP = 3;			//增量每次除以3

template<class Elem>
void shellsort(Elem A[],int n)
{
    for(int incr = n / INCRGAP;incr > 0;incr /= INCRGAP){//incr表示每次划分子序列的增量，例如n=24，则incr增量就是8，隔八个连一个
        for(int i = 0;i < incr;i++){//增量是几就划分了几组
            /*对每一组子序列进行插入排序，当增量为1时，对所有元素进行最后一次插入排序*/
...
当选择“增量每次除以3”递减时，Shell排序的**平均运行时间是O(n^(1.5))**
...
>
> 其实，对于rand()的范围，我们是可以进行人为设定的，只需要在宏定义中定义一个random(int x)函数，就可以生成范围为0至x的随机数值。当然，也可以定义为random(a,b)，使其生成范围为a至b的随机数值。具体定义方法在[通式](https://www.cnblogs.com/S031602240/p/6391960.html#3)部分。
...
//2.两个下标分别从首尾向中间扫描的方法
template <class Elem>
int partition1(Elem A[],int low,int high){
	Elem pivot=A[low];			//取当前表的第一个元素为枢轴值，对表进行划分
...
```c++
template<class Elem>
void mergesortcore(Elem A[],Elem temp[],int i,int j)	//temp为辅助数组
{
    if(i == j)  return;
    int mid = (i + j)/2;

    mergesortcore(A,temp,i,mid);			//左闭右闭区间
...
![1565609266267](pic/1565609266267.png)
...
堆排序首先**根据数组构建最大堆**，然后每次“删除”堆顶元素（将堆顶元素移至末尾）。最后得到的序列就是从小到大排序的序列

![1565609283666](pic/1565609283666.png)
...
![1565609309501](pic/1565609309501.png)
